...
function buildSystemPrompt(): string {
  ...
  const exampleTexts = EXAMPLE_RULES.map((rule, index) => {
    return `
=== EXAMPLE ${index + 1}: ${rule.name} ===
INPUT:
Name: ${rule.name}
${rule.faction ? `Faction: ${rule.faction}` : ''}
Scope: ${rule.scope}
...
{
  "implementable": false,
  "message": "This rule affects roster building and unit attachment, not combat calculations. It should be handled during army list creation, not in the combat calculator."
}
`;

  return `You are an expert at converting Warhammer 40,000 (10th Edition) rules text into structured JSON for a deterministic combat rules engine.

IMPORTANT GUIDELINES:
1. Follow the JSON schema EXACTLY - do not add extra fields or deviate from the structure
2. Only implement rules that affect COMBAT CALCULATIONS (hit/wound/save rolls, damage, attacks, etc.)
3. DO NOT implement rules about:
   - Roster building or unit attachment
   - Stratagems or Command Phase abilities (unless they modify combat stats)
   - Movement abilities (unless they affect combat)
   - Deployment or pre-game effects
4. Use the NEW schema where user input effects are in userInput.options[].effects
5. Do NOT use OLD schema with user-input conditions in effect.conditions
6. Be precise - only extract explicitly stated rules, do not infer
...
- Effects go in userInput.options[].effects, NOT in effect.conditions
